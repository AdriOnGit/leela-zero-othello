...
// Dirichlet noise technique: stochastic exploration during the node selection phase of the search tree.
// Guarantees greater diversification in exploring game states.
void UCTNode::dirichlet_noise(const float epsilon, const float alpha) {
    ...
    child_cnt = 0;
    // For each child calculate a new policy value using the Dirichlet noise formula.
    for (auto& child : m_children) {
        auto policy = child->get_policy();
        auto eta_a = dirichlet_vector[child_cnt++];
        // Mix the original policy value with the Dirichlet noise based on epsilon's value.
...
void UCTNode::randomize_first_proportionally() {
    auto accum = 0.0; // Keeps track of the cumulative sum of the proportional probabilities of each child during each loop iteration.
    ...
    for (const auto& child : m_children) {
        ...
        if (norm_factor == 0.0) {
            ...
            if (visits <= cfg_random_min_visits) {
                // The child node has number of visits less than the minimum threshold.
                return;
            }
        }
        if (visits > cfg_random_min_visits) {
    ...
    auto index = size_t{0};
    // Look for the index of the first element that goes over the random number "pick".
...
void UCTNode::prepare_root_node(Network& network, const int color,
    ...
    } else { // The node was just expanded so calculate the evaluation.
