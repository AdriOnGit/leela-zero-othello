...
// This file defines two classes declared in the header file: 
// - AnalyzeTags: used to parse the configuration tags;
// - GTP: used to handle play;

// Configuration flags
bool cfg_gtp_mode; // Indicates whether the AI is operating in GTP mode.
bool cfg_allow_pondering; // Determines if the AI can think during the opponent's turn.
unsigned int cfg_num_threads; // Specifies the number of threads used for parallel processing.
...
AnalyzeTags::AnalyzeTags(std::istringstream& cmdstream, const GameState& game) {
    ...
    while (true) {
        ...
        // Parses the avoid and allow commands, which take a vertex rectangle in the form of A1:B1, and allows or denies all moves within that range.
        if (tag == "avoid" || tag == "allow") {
            std::string textcolor, textmoves;
            size_t until_movenum;
            cmdstream >> textcolor;
            cmdstream >> textmoves; // Contains a string of moves seperated by a comma.
            ...
            while (!movestream.eof()) {
                std::string textmove;
                getline(movestream, textmove, ','); // Seperates the moves, each move pair is put into textmove.
                ...
                if (sepidx != std::string::npos) { // If the semicolon is found.
                    ...
                    for (auto move_x = xmin; move_x <= xmax; move_x++) { // Iterates all the vertexes specified within the range (rectamgle defined by input positions).
...
// Checks if a move is to be avoided.
...
// Checks if there are any moves to avoid or allow.
bool AnalyzeTags::has_move_restrictions() const {
    return !m_moves_to_avoid.empty() || !m_moves_to_allow.empty();
}

// Declares pointer to the network.
std::unique_ptr<Network> GTP::s_network;

// Gets the pointer to the network and sets the max memory parameters, initializing the GTP.
...
// List of all commands (not all of them are included!).
...
void GTP::execute(GameState& game, const std::string& xinput) {
    ...
    if (input == "") {
        ...
        return;
    } else if (std::isdigit(input[0])) { // Seperates the id from the command if there is an id in front.
...
// Sets up other options, like maximum number of visits, playouts, lagbuffer, enable or disable pondering, resign percentage, etc.
