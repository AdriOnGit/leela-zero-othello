...
public:
    OutputAnalysisData(std::string move, const int visits, const float winrate,
        ...
        : m_move(std::move(move)),
          ...
          m_pv(std::move(pv)), // Principal Variation - sequence of moves that are considered the best.
          m_lcb(lcb), // Lower Confidence Bound - estimate of the real performance of a move.
          m_lcb_ratio_exceeded(lcb_ratio_exceeded) {} // Indicates if LCB ratio was exceeded. 
    ...
    // If at least one has not exceeded, checks if they have the same visits. In this case, it checks winrate.
    // If they have different visits, it checks those.
...
bool UCTSearch::advance_to_new_rootstate() {
    ...
    for (auto i = 0; i < depth; i++) {
        ...
        // Test that contains the state of the root, plays a new move and updates the state.
        test->forward_move();
        // The move is saved.
...
void UCTSearch::update_root() {
    ...
    // If the advancement to the new root doesn't work, or if the new root is null then create a new standard root node.
...
SearchResult UCTSearch::play_simulation(GameState& currstate,
    ...
    // If the node is expandable but there have been more than 2 passes, then interrupt and return the result until that point.
    if (node->expandable()) {
...
void UCTSearch::dump_stats(const FastState& state, UCTNode& parent) {
    // Doesn't print anything if "quiet" mode is activated, or if the parent doesn't have any children.
    ...
    // Checks all children to determine the maximum number of visits between them (used in sorting).
    auto max_visits = 0;
...
void UCTSearch::tree_stats(const UCTNode& node) {
    ...
    std::function<void(const UCTNode& node, size_t)> traverse =
        [&](const UCTNode& node, size_t depth) {
            nodes += 1;
            // Incremented only if it has more than one visit (a leaf node can't be visited more than once).
...
bool UCTSearch::should_resign(const passflag_t passflag, const float besteval) {
    ...
    // If the percentage of resign is the default one, the threshold will be 10% of that value.
    // Otherwise it will be equal to the percentage specified by the user.
...
int UCTSearch::think(const int color, const passflag_t passflag) {
    ...
    do {
        ...
        // Prints the information if there is a time interval analysis and enough time has elapsed since the last analysis.
