...
bool UCTNode::create_children(Network& network, std::atomic<int>& nodecount,
    ...
    if (state.get_passes() >= 2) { // There were at least two consecutive passes - so no successors.
        return false;
    }
    ...
    for (auto i = 0; i < NUM_INTERSECTIONS; i++) {
        ...
        // If the move is legal, add it to legal sum - the sum of the probabilities of all legal moves.
...
// Connects child nodes to the current node (only if they exceed a certain policy probability threshold).
...
void UCTNode::update(const float eval) {
    ...
    // If the node has at least one visit, calculate the difference between the new evaluation the average of the previous ones.
...
// min_psa_ratio = minimum threshold of the probability of children selection during expansion.
