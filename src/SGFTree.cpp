...
GameState SGFTree::follow_mainline_state(const unsigned int movenum) const {
    ...
    // If a time controller is defined, it must be also applied to the game state represented by result.
    ...
    for (unsigned int i = 0; i <= movenum && link != nullptr; i++) {
        ...
        if (i != 0) {
            ...
            if (colored_move.first != FastBoard::INVAL) {
                ...
                // 2) The coordinate is checked to prevent a move being performed on an occupied cell.
                // 3) The game state is checked to guarantee that no moves are performed on an occupied cell.
...
void SGFTree::populate_states() {
    ...
    // If it is Go but the board size is not defined (SZ), then insert the standard size 19x19.
    ...
    // komi
    // Handles Komi: points given to the second player because of the disadvantage.
    // Looks for the komi value in the file, converts it in float and uses it to initialize the game along with the board size.
    ...
    // time
    // Looks for the main time and other properties relative to the time controller.
    ...
    // result
    // If the string result contains the word "Time", then the result is not available (EMPTY).
    ...
    // If there is a handicap, but the value is not specified, check the first child node to find it.
    ...
    // After having copied the game state to the child, apply the associated move and proceed recursively exploring the subtree using populate_states().
...
void SGFTree::apply_move(const int color, const int move) {
    ...
    if (move != FastBoard::PASS && move != FastBoard::RESIGN) {
        ...
        // Checks if this cell of the board is occupied by the opposite color of whoever is playing,
        // or if it's in an invalid cell.
...
std::string SGFTree::state_to_string(GameState& pstate, const int compcolor) {
    ...
    *state = pstate;
    ...
    // File with weights - add the first 8 characters of the weights file name to leela's name.
    ...
    } else { // Player who resigned (if any).
