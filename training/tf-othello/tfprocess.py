...
class TFProcess:
    ...
    def init(self, batch_size, macrobatch=1, gpus_num=None, logbase='leelalogs'):
        ...
        planes = tf.reshape(planes, (batch_size, 18, 8*8))
        probs = tf.reshape(probs, (batch_size, 8*8 + 1))
    ...
    def init_net(self, planes, probs, winner, gpus_num):
        ...
        # Summary part
        num_generations = len([f for f in os.listdir(leela_logs + "/test") if f[0].isdigit()]) + 1
        gen_dir = f"{num_generations}gen"
        test_path = leela_logs + "/test" + f"/{gen_dir}"
        train_path = leela_logs + "/train" + f"/{gen_dir}"
        os.makedirs(test_path, exist_ok=True)
        os.makedirs(train_path, exist_ok=True)
        self.test_writer = tf.summary.FileWriter(
            test_path, self.session.graph)
        self.train_writer = tf.summary.FileWriter(
            train_path, self.session.graph)
    ...
    def construct_net(self, planes):
        ...
        W_fc1 = weight_variable("w_fc_1", [2 * 8 * 8, (8 * 8) + 1], self.model_dtype)
        b_fc1 = bias_variable("b_fc_1", [(8 * 8) + 1], self.model_dtype)
        ...
        W_fc2 = weight_variable("w_fc_2", [8 * 8, 256], self.model_dtype)
...
class TFProcessTest(unittest.TestCase):
    def test_can_replace_weights(self):
        ...
        data.append([0.4] * 2*8*8 * (8*8+1))
        data.append([0.5] * (8*8+1))
        # value
        data.extend(gen_block(1, tfprocess.residual_filters, 1))
        data.append([0.6] * 8*8 * 256)
        data.append([0.7] * 256)
        data.append([0.8] * 256)
        data.append([0.9] * 1)
